...
inline socket_t Server::create_server_socket(const std::string &host, int port,
                                            int socket_flags,
                                            SocketOptions socket_options) const {
  socket_t sock = detail::create_socket(
      host, std::string(), port, address_family_, socket_flags, tcp_nodelay_,
      std::move(socket_options),
      [](socket_t sock, struct addrinfo &ai) -> bool {
        printf("Binding the port for the socket\n");
        if (::bind(sock, ai.ai_addr, static_cast<socklen_t>(ai.ai_addrlen))) {
          perror("Bind failed: ");
          return false;
        }
        printf("Port binded\n");
        printf("Putting the socket in listening mode\n");
        if (::listen(sock, CPPHTTPLIB_LISTEN_BACKLOG)) { 
          perror("Listen failed: ");
          return false; 
        }
        printf("Socket listening for petitions\n");
        return true;
      });
...
inline int Server::bind_internal(const std::string &host, int port,
  ...
  if (port == 0) {
    ...
    if (addr.ss_family == AF_INET) {
      int bound_port = ntohs(reinterpret_cast<struct sockaddr_in *>(&addr)->sin_port);
      printf("Bound to port %d\n", bound_port);
      return bound_port;
    } else if (addr.ss_family == AF_INET6) {
      int bound_port = ntohs(reinterpret_cast<struct sockaddr_in6 *>(&addr)->sin6_port);
      printf("Bound to port %d\n", bound_port);
      return bound_port;
    } else {
    ...
    printf("Bound to specified port %d\n", port);
    return port;
  }
}
